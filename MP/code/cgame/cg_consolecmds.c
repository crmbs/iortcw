...
static void CG_Viewpos_f (void) {
	CG_Printf ("(%f %f %f) : %f %f %f : %i\n", (float)cg.refdef.vieworg[0],
		(float)cg.refdef.vieworg[1], (float)cg.refdef.vieworg[2], 
		(float)cg.refdefViewAngles[YAW], (float)cg.refdefViewAngles[PITCH], (float)cg.refdefViewAngles[ROLL],(int)cg.time);
}
...
static void Cmd_ExecAtTime_f (void) {
	...
	execString = buffer;	
	trap_Cvar_Set("demo_execAtTimeTime",va("%i\n",execTime));
	trap_Cvar_Set("demo_execAtTime","1");
	trap_Cvar_Set("demo_execAtTimeVal",va("%s\n",execString));
...
static void CG_DemoCam( void ) {
	...
	cg.demoCamSpeed = demo_freecamSpeed.value;
