...
// JPW NERVE per atvi req
		...
		case MOD_DYNAMITE_SPLASH:
			if (demo_drawVaporise.integer) message = "dynamited himself to pieces";
			break;
// jpw
		case MOD_GRENADE_SPLASH:
			if (demo_drawVaporise.integer) message = "dove on his own grenade";
			break;
		case MOD_ROCKET_SPLASH:
			if (demo_drawVaporise.integer) message = "vaporized himself";
			break;
		case MOD_AIRSTRIKE:
			if (demo_drawVaporise.integer) message = "obliterated himself";
			break;
			//case MOD_BFG_SPLASH:
			//message = "should have used a smaller gun";
			//break;
		case MOD_EXPLOSIVE:
			if (demo_drawVaporise.integer) message = "died in his own explosion";
			break;
		default:
	...
	if (message) {
		...
		if( demo_drawMyFrags.integer && killtype == 2) 
		{
			message3 = va("%s %s.\n", targetName, message);
	...
	if ( attacker == cg.snap->ps.clientNum ) {
		...
		if ( ci->team == ca->team && demo_drawTeamkills.integer == 0){
			
		}else{
			if(demo_freecamConsole.integer == 2 || demo_freecamConsole.integer == 0) CG_PriorityCenterPrint( s, SCREEN_HEIGHT * 0.75, BIGCHAR_WIDTH * 0.6, 1 );
		}
...
// JPW NERVE if attacker != target but on same team
		if (ci->team == ca->team) {
			if ( demo_drawTeamkills.integer) message = "^1WAS KILLED BY TEAMMATE^7";
			message2="";
		}
// jpw
		...
		if (message) {
			...
			if( demo_drawMyFrags.integer && (cg.snap->ps.clientNum == attacker)) 
			{
