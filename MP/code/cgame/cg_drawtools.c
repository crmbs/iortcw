...
void CG_DrawStringExt( int x, int y, const char *string, const float *setColor, 
	...
	shadowX = 2;
	if (demo_drawShadowedPopupsX.integer) {
		shadowX = demo_drawShadowedPopupsX.integer;
	}

	shadowY = 2;
	if (demo_drawShadowedPopupsY.integer) {
		shadowY = demo_drawShadowedPopupsY.integer;
	}
	...
	if (shadow) {
		...
		color[3] = setColor[3];
		if (demo_drawShadowedPopupsAlpha.value > 0) {
			color[3] = demo_drawShadowedPopupsAlpha.value * setColor[3];
		}
		...
		while ( *s && cnt < maxChars) {
			...
			CG_DrawChar( xx + shadowX, y + shadowY, charWidth, charHeight, *s );
			cnt++;
			xx += charWidth;
			s++;
		}
