...
static float CG_DrawTimer( float y ) {
	...
	w = CG_DrawStrlen( s ) * 12;

	//CG_DrawBigString( UPPERRIGHT_X - w, y + 2, s, 1.0F);

	CG_DrawStringExt( 640 - w - 4, y + 6, s, hcolor, qfalse, qtrue, 12, 12, 20 );
	...
	for (i = 0; i < numSortedTeamPlayers; i++) {
		...
		if ( ci->infoValid && ci->team == cg.snap->ps.persistant[PERS_TEAM]) {
			...
			if (lwidth) {
				...
				CG_DrawStringExt( xx, y,
					p, hcolor, qfalse, qtrue, TINYCHAR_WIDTH, TINYCHAR_HEIGHT,
			...
			Com_sprintf (st, sizeof(st), "%3i", health); // JPW NERVE pulled class stuff since it's at top now
...
static void CG_DrawNotify( void ) {
	...
	if (cgs.notifyLastPos != cgs.notifyPos) {
		...
		for (i = cgs.notifyPos - 1; i >= cgs.notifyLastPos; i--) {
			...
			if ( demo_drawShadowedPopups.integer ) blub = qtrue;
			else blub = qfalse;			
...
// JPW NERVE added round timer
	...
	y = NOTIFYLOC_Y;
	if (demo_numPopups.integer && demo_popupHeight.integer) {
		y = (demo_numPopups.integer * demo_popupHeight.integer) + 40;
	}
...
static void CG_Draw2D( void ) {
	...
	if ( cg_draw2D.integer == 0 ) {
		if (demo_drawCrosshair.integer) CG_DrawCrosshair();
		if (demo_drawPopups.integer) {
			CG_DrawCenterString();
			CG_DrawNotify();
		//CG_DrawObjectiveInfo();
	...
	if (demo_drawSimpleStatus.integer) {
		...
		CG_FillRect(0, 440, 640, 40, hcolor);
	}
	...
	if ( !CG_DrawScoreboard() ) {
		...
		// NERVE - SMF
		if ( cg_drawCompass.integer && !demo_drawSimpleStatus.integer)
			CG_DrawCompass();
...
void CG_DrawActive( stereoFrame_t stereoView ) {
	...
	if (!cg.demoCam) CG_ShakeCamera();		// NERVE - SMF
