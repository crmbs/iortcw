...
static void CG_DrawSimplePlayerWeaponIcon( rectDef_t *rect, qboolean drawHighlighted, int align ) {
	int			size;
	int			realweap;			// DHM - Nerve
	qhandle_t	icon;
	float		scale,halfScale;
...
static void CG_DrawSimplePlayerAmmoValue(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle, int type) {
	int ammovalue, ammovalue2=0, clipvalue, clipvalue2 = 0;
	centity_t	*cent;
	playerState_t	*ps;
	int		weap, startx;
...
//		case WP_DYNAMITE: // JPW NERVE these recharge
	...
	// NERVE - SMF
	if ( !skipammo )
		startx = CG_DrawFieldWidth( rect->x + xOffset, rect->y + yOffset, 4, ammovalue, 20*scale, 32*scale );
	else
		startx = 0;
	...
	if ( ammovalue > -1 && type == 0 ) {
		if (shader) {
			trap_R_SetColor( color );
			CG_DrawPic(rect->x + xOffset, rect->y + yOffset, rect->w, rect->h, shader);
			trap_R_SetColor( NULL );
		} else {
			trap_R_SetColor(color);

			// draw the clip count
			CG_DrawField( rect->x + xOffset, rect->y + yOffset, 4, ammovalue, 20*scale, 32*scale, qtrue, qfalse );
		}

		// draw the slash
		if ( type == 0 ) {
			CG_DrawPic( rect->x - startx - 16 + xOffset, rect->y + yOffset, 16, 28, trap_R_RegisterShader( "gfx/2d/numbers/slash" ) );
		}
	}
	...
	if ( clipvalue > -1 && type == 1 ) {
		if (shader) {
			...
			// draw the clip count
			CG_DrawField( rect->x - startx + xOffset, rect->y + yOffset, 4, clipvalue, 20*scale, 32*scale, qtrue, qfalse );
		}
	}
}
...
static void CG_DrawSimplePlayerHealth(rectDef_t *rect, float scale, vec4_t color, qhandle_t shader, int textStyle ) {
	...
	if (shader) {
		...
		trap_R_SetColor(color);
		CG_DrawField( rect->x + xOffset, rect->y + yOffset, 3, value, 20*scale, 32*scale, qtrue, qtrue );			// NERVE - SMF
		...
		if ( ps->persistant[PERS_RESPAWNS_LEFT] >= 0 ) {
			...
			if ( cg.snap->ps.persistant[PERS_TEAM] == TEAM_BLUE )
				CG_DrawPic( rect->x + 92 + xOffset, rect->y-2 + yOffset, 64, 32, cgs.media.hudAlliedHelmet );
			else
				CG_DrawPic( rect->x + 92 + xOffset, rect->y-2 + yOffset, 64, 32, cgs.media.hudAxisHelmet );

			trap_R_SetColor(color);
			CG_DrawField( rect->x + 144 + xOffset, rect->y + yOffset, 3, ps->persistant[PERS_RESPAWNS_LEFT], 20*scale, 32*scale, qtrue, qtrue );
		}

		if ( cgs.clientinfo[cg.snap->ps.clientNum].powerups & (1 << PW_INVULNERABLE) ) {
			CG_DrawPic( rect->x+56 + xOffset, rect->y-6 + yOffset, 32, 32, cgs.media.spawnInvincibleShader );
		}
		// dhm
	}
}
...
static void CG_DrawSimpleFatigue(rectDef_t *rect, vec4_t color, int align) {
	//	vec4_t	color = {0, 1, 0, 1}, color2 = {1, 0, 0, 1};
	vec4_t	colorBonus = {1, 1, 0, 0.45f};	// yellow (a little more solid for the 'bonus' stamina)
	float	barFrac;//, omBarFrac;
	int		flags = 0;
...
//	omBarFrac = 1.0f-barFrac;
	...
	CG_FilledBar(626, 435, rect->w, rect->h*0.82f, color, NULL, NULL, (float)cg.snap->ps.sprintTime / SPRINTTIME, flags);
...
static void CG_DrawSimpleWeapRecharge(rectDef_t *rect, vec4_t color, int align) {
	float	barFrac;
	float	chargeTime;
	int		weap = 0;
	int		flags = 0;
...
// JPW NERVE -- added drawWeaponPercent in multiplayer
	if (cgs.gametype >= GT_WOLF) {
		...
		CG_FilledBar(620, 445, rect->w, rect->h*0.84f, color, NULL, bgcolor, barFrac, flags);
...
void CG_OwnerDraw(float x, float y, float w, float h, float text_x, float text_y, int ownerDraw, int ownerDrawFlags, int align, float special, float scale, vec4_t color, qhandle_t shader, int textStyle) {
  ...
  switch (ownerDraw) {
	case CG_PLAYER_WEAPON_ICON2D:
		if (!demo_drawSimpleStatus.integer)
	...
	case CG_PLAYER_AMMO_VALUE:
		if (demo_drawSimpleStatus.integer) {
			CG_DrawSimplePlayerAmmoValue(&rect, scale, color, shader, textStyle, 0);
		}
		else {
	...
	case CG_PLAYER_AMMOCLIP_VALUE:
		if (demo_drawSimpleStatus.integer) {
			CG_DrawSimplePlayerAmmoValue(&rect, scale, color, shader, textStyle, 1);
		}
		else {
	...
	case CG_PLAYER_WEAPON_STABILITY:
		...
		break;
	case CG_STAMINA:
		//CG_DrawSimpleFatigue(&rect, color, align);
		break;
	// DHM - Nerve
	case CG_PLAYER_WEAPON_RECHARGE:
		if (demo_drawSimpleStatus.integer) {
			CG_DrawSimpleWeapRecharge(&rect, color, align);
		}
		else {
	...
	case CG_PLAYER_HEALTH:
		if (demo_drawSimpleStatus.integer) {
			CG_DrawSimplePlayerHealth(&rect, scale, color, shader, textStyle);
		}
		else {
