            ...
            {  // hdrl avi list
                ...
                START_CHUNK(afd, "LIST"); {
                    ...
                    START_CHUNK(afd, "ISFT"); {
                        ...
                        WRITE_STRING(va("iortcw version %s", "UNKNOWN"));
                    }
                    END_CHUNK(afd);
                }
                END_CHUNK(afd);
...
qboolean CL_OpenAVIForWriting(aviFileData_t* afd, const char* fileName, qboolean us, qboolean avi, qboolean noSoundAvi, qboolean wav, qboolean tga, qboolean jpg, qboolean png)
{
...
static qboolean CL_CheckRiffSize(aviFileData_t* afd, int bytesToAdd)
{
    ...
    if (newRiffSize > afd->newRiffOrCloseFileSize)
    {
        ...
        if (!afd->useOpenDml) {
            ...
            CL_OpenAVIForWriting(afd, afd->givenFileName, qtrue, afd->avi, afd->noSoundAvi, afd->wav, afd->tga, afd->jpg, afd->png);
            return qtrue;
        }
        else {
            //FIXME
